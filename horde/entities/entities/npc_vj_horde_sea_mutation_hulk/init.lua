...
ENT.MeleeAttackKnockBack_Forward2 = 160
ENT.MeleeAttackKnockBack_Up1 = 280
ENT.MeleeAttackKnockBack_Up2 = 310
...
function ENT:CustomOnInitialize()
    ...
    self.WorldShakeOnMoveAmplitude = 10
    self.WorldShakeOnMoveRadius = 250
    self.WorldShakeOnMoveDuration = 0.4
    self.WorldShakeOnMoveFrequency = 100
    self.Sea_Enraged = false
end
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:CustomOnThink()
    if not self.Sea_Enraged and self:Health() < self:GetMaxHealth() / 2 then
        self.Sea_Enraged = true
        self.MeleeAttackDamage = self.MeleeAttackDamage * 1.4
        self:EmitSound("npc/zombie_poison/pz_alert1.wav", 500, 70, 1, CHAN_STATIC)
    end
end

VJ.AddNPC("Mutation Hulk", "npc_vj_horde_sea_mutation_hulk", "Sea-Infection")
