...
function ENT:CustomOnInitialize()
    ...
    self.Sea_ChantCooldown = 0
end
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:CustomOnThink()
    if CurTime() > self.Sea_ChantCooldown and IsValid(self:GetEnemy()) then
        self.Sea_ChantCooldown = CurTime() + 22
        self:EmitSound("npc/zombie/zombie_alert1.wav", 500, 60, 1, CHAN_STATIC)
        -- Buff nearby allied NPCs with brief speed increase (simulated)
        for _, ent in pairs(ents.FindInSphere(self:GetPos(), 450)) do
            if IsValid(ent) and ent:IsNPC() and ent ~= self then
                local npcClass = ent.VJ_NPC_Class
                if npcClass and table.HasValue(npcClass, "CLASS_ZOMBIE") then
                    -- Nudge their velocity forward toward enemy
                    local enemy = ent:GetEnemy()
                    if IsValid(enemy) then
                        local dir = (enemy:GetPos() - ent:GetPos()):GetNormalized()
                        ent:SetLocalVelocity(ent:GetLocalVelocity() + dir * 120)
                    end
                end
            end
        end
...
VJ.AddNPC("Tidecaller", "npc_vj_horde_sea_tidecaller", "Sea-Infection")
