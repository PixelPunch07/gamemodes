...
function ENT:SpawnFunction(ply, tr)
	...
	local ent = ents.Create("horde_net")

		ent:SetPos(SpawnPos)
		ent:SetAngles(SpawnAng)
	ent:Spawn()
		ent:SetModelScale( ent:GetModelScale() * 0.85, 0.001 )
...
function ENT:Think()
			if SERVER then 
  local entities = ents.FindInSphere(self:GetPos(), 5.5)
...
if timer.TimeLeft("npctrip"..ent:EntIndex()) == nil then
    ...
    self:EmitSound("spy_item_preview_spy_pl_fallpain.wav")
    ...
    ent:SetAngles(ent:GetAngles() + Angle(-85,0,0))
    dmgInfo:SetDamage(math.random(3,6))
    ent:TakeDamageInfo(dmgInfo)
    self:TakeDamage(5)
    timer.Simple(1.2, function()
        if ent:IsValid() then
            ent:SetMoveDelay(original)
            ent:SetAngles(ent:GetAngles() + Angle(85,0,0))
        end
    end)
    timer.Create("npctrip"..ent:EntIndex(), 3, 1, function() end)
end
					else
...
function ENT:PhysicsCollide(data, phys)
	...
	if data.DeltaTime > 0.2 then
		self:EmitSound("spy_taunt_tuefort_tango_taunt_yeti_standee_spy_clothes_rustle.wav")
